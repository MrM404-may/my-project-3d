...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False,  ape_code=-1):
    ...
    # 获取相机所属区域
    camera_region = viewpoint_camera.region_idx
    
    # 只保留属于当前区域的锚点高斯
    region_mask = (pc._region == camera_region)
    # 结合视锥体掩码和区域掩码
    combined_mask = visible_mask & region_mask
    ...
    if pc.add_opacity_dist:
        opacity_input = cat_local_view
    else:
        opacity_input = cat_local_view_wodist
    # 使用forward_opacity_mlp方法，传入camera_region参数
    neural_opacity = pc.forward_opacity_mlp(opacity_input, camera_region) # [N, k]
    ...
    # 使用forward_color_mlp方法，传入camera_region参数
    color = pc.forward_color_mlp(color_input, camera_region)
    color = color.reshape([anchor.shape[0]*pc.n_offsets, 3])# [mask]

    # get offset's cov
    if pc.add_cov_dist:
        cov_input = cat_local_view
    else:
        cov_input = cat_local_view_wodist
    # 使用forward_cov_mlp方法，传入camera_region参数
    scale_rot = pc.forward_cov_mlp(cov_input, camera_region)
