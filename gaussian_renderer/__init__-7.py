...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False,  ape_code=-1):
    ...
    # 使用forward_opacity_mlp方法，传入region参数
    neural_opacity = pc.forward_opacity_mlp(opacity_input, region) # [N, k]
    ...
    # 使用forward_color_mlp方法，传入region参数
    color = pc.forward_color_mlp(color_input, region)
    ...
    # 使用forward_cov_mlp方法，传入region参数
    scale_rot = pc.forward_cov_mlp(cov_input, region)
