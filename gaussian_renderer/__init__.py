...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False,  ape_code=-1, camera_region=0):
    ...
    if pc.appearance_dim > 0:
        if pc.add_color_dist:
            color = pc.get_color_mlp(camera_region)(torch.cat([anchor, ob_view, ob_dist, level, appearance], dim=1))
        else:
            color = pc.get_color_mlp(camera_region)(torch.cat([anchor, ob_view, level, appearance], dim=1))
    else:
        if pc.add_color_dist:
