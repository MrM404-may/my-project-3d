...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False,  ape_code=-1):
    ...
    if pc.appearance_dim > 0:
        if is_training or ape_code < 0:
            # print("Training mode or no ape code provided, using camera indices for appearance embedding.")
            camera_indicies = torch.ones_like(cat_local_view[:,0], dtype=torch.long, device=ob_dist.device) * viewpoint_camera.uid
            appearance = pc.get_appearance(camera_indicies)
        else:
            # print("Training mode  ape code provided, using camera indices for appearance embedding.")
    ...
    if pc.appearance_dim > 0:
        if pc.add_color_dist:
            color = pc.get_color_mlp(torch.cat([anchor, ob_view, ob_dist, level, appearance], dim=1))
        else:
            color = pc.get_color_mlp(torch.cat([anchor, ob_view, level, appearance], dim=1))
    else:
        if pc.add_color_dist:
