...
class CameraInfo:
    def __init__(self, uid: int, R: np.array, T: np.array, FovY: np.array, FovX: np.array, image_path: str, image_name: str, width: int, height: int, image=None):
        self.uid = uid
        self.R = R
        self.T = T
        self.FovY = FovY
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, 
                              ...
                              width=width, height=height, image=None)
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if fovx is not None:
                ...
                FovY = focal2fov(frame["fl_y"], image_size[1])
                FovX = focal2fov(frame["fl_x"], image_size[0])
            
            # 创建CameraInfo对象，不立即加载图像
            cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, 
                                  image_path=image_path, image_name=image_name, 
                                  width=image_size[0], height=image_size[1], image=None)
            
            # 重写image属性的getter方法，添加图像处理逻辑
            original_image_getter = cam_info.image.__get__
            
            def custom_image_getter(instance):
                if instance._image is None:
                    from PIL import Image
            ...
            # 替换image属性的getter
            cam_info.image = property(custom_image_getter, cam_info.image.__set__)
