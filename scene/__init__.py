...
class Scene:
    ...
    def getTrainCameras_batch_idx(self, batch_idx=None, batch_size=None):
        """
        获取训练相机
        ...
        if batch_idx is not None and self.batch_size:
            # 确保指定批次已加载
            if batch_idx * self.batch_size >= len(self.train_cameras):
                return []
            ...
            end_idx = min(start_idx + self.batch_size, len(self.train_cameras))
            all_cams = []
            for scale in self.resolution_scales:
