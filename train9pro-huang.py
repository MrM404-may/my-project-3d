...
# ====================== 多区域配置加载 (核心改造) ======================
    # 配置路径（可根据需要改为命令行参数）
    JSON_PATH = r"/root/autodl-tmp/Octree-GS/Octree-GS/data/Huang0321/cameras.json"
    DEPTH_DIR = r"/root/autodl-tmp/Octree-GS/Octree-GS/data/Huang0321/images_metric3"
    MASK_DIR = r"/root/autodl-tmp/Octree-GS/Octree-GS/data/Huang0321/images_mark2"
    REGIONS_JSON_PATH = r"/root/autodl-tmp/Octree-GS/Octree-GS/data/Huang0321/regions_config.json"  # 多区域配置文件
    CACHE_JSON_PAH = r"/root/autodl-tmp/Octree-GS/Octree-GS/data/Huang0321/cache.json"
    MAX_CAMERA_ID = 300000
    MY_CAMERA_INTRINSICS = (
        267.71846329403036,267.5260126510565,316.70833751877217,178.00875728823831, 
    )
    ...
    # 【需求1】初始化时加载第一个区域照片到GPU
    print(f"[Init] 加载第一个区域 ({current_region['name']}) 的相机到 GPU...")
    ...
    for iteration in range(first_iter, TOTAL_TRAIN_ITER + 1):        
        ...
        if iteration in region_switch_points:
            ...
            if new_region_idx != current_region_idx:
                ...
                # 【需求2】释放上个区域的照片在CPU或者GPU内存
                print(f"[Region Switch] 释放区域 {prev_region_idx+1} ({prev_region_config['name']}) 的相机内存...")
                scene.release_region_cameras(REGION_CAMERA_POOLS[prev_region_idx])
                ...
                current_camera_pool = REGION_CAMERA_POOLS[current_region_idx]
                print(f"[Region Switch] 加载新区域 {current_region_idx+1} ({current_region['name']}) 的相机到 GPU...")
                scene.load_region_cameras(current_camera_pool)
        ...
        # # 遍历所有 anchor，为指定时刻初始化特征
        # for i in range(gaussians._anchor.shape[0]):
        #     r = gaussians._region[i].item()
        #     key = (r, moment)
        #     if key not in gaussians._anchor_feat_dict[i]:
        #         # 复制默认特征作为初始值
        #         init_feat = gaussians._anchor_feat[i].clone().detach().requires_grad_(True)
        #         gaussians.set_anchor_feat_at_moment(moment=moment, region=r, anchor_idx=i, feat=init_feat)
        # # 注意：如果在训练中动态添加，可能需要重新调用 training_setup 或手动添加到 optimizer
        
        # 传递当前区域索引和时刻参数
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, visible_mask=voxel_visible_mask, retain_grad=retain_grad, camera_region=current_region_idx, moment=moment)
        ...
        with torch.no_grad():
            ...
            if current_local_iter < opt.update_until and current_local_iter > opt.start_stat:
                ...
                # 改为：
                if opt.update_anchor and current_local_iter > opt.update_from and current_local_iter % (opt.update_interval*10) == 0:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    # 【新增】测试时也使用相机 uid 作为时刻
                    test_moment = viewpoint.uid
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs, visible_mask=voxel_visible_mask, moment=test_moment)["render"], 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[i * 40000 for i in range(1, 65 + 1)])
